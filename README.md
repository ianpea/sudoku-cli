...
The `check` command detects duplicate non-zero values in each:
...
Cells are then removed in random order. After each removal, the number of possible solutions is counted using backtracking. A removal is retained only if exactly one solution remains.
...
The application separates CLI concerns, game orchestration, domain state, move history, puzzle generation, and Sudoku validation so that the core game logic is not coupled to console I/O.
